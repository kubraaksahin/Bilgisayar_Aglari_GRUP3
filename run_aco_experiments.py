...
def path_min_capacity_mbps(G, path) -> float:
    """Bottleneck capacity on path (min capacity_mbps)."""
...
def run_single_demand(G, evaluator, demand, aco_params, weights, seed: int):
...
def tune_hyperparams_random(
    ...
    for t in range(1, trials + 1):
        ...
        for di, d in enumerate(tune_demands):
            best_cost_for_demand = float("inf")

            for r in range(repeats):
                total_runs += 1
                path, details = run_single_demand(
                    G, evaluator, d, params, weights,
                    seed=10_000 + 97 * t + 31 * di + r
                )
                if path is None:
                    continue
...
def evaluate_demands_simplified(
    ...
    for i, d in enumerate(used, start=1):
        ...
        for r in range(repeats):
            path, details = run_single_demand(
                G, evaluator, d, aco_params, weights,
                seed=50_000 + 131 * i + r
            )
            if path is None:
                continue
